...
var App = new Vue({
    ...
    methods: {
        ...
        lookFor: function lookFor() {
            ...
            $.ajax({
                url: '/Training/AttendanceStaticSearch',
                ...
                success: function (data) {
                    ...
                    if (data.length > 0) {
                        ...
                        tableStr += "<tr><td></td >";
                        for (var i in data) {
                            tableStr += '<td style="font-weight:bold" colspan="' + data[i].list.length + '">' + App.ChangeDateFormat(data[i].TrainingDate) + '</td>';
                        }
                        tableStr += "  </tr><tr><td style='font-weight:bold'>统计项</td>  ";
                        for (var j in data) {
                            for (var k in data[j].list) {
                                tableStr += ' <td style="font-weight:bold">时间段' + (parseInt(k) + 1) + '</td>';
                            }
                        }
                        tableStr += "  </tr><tr><td style='font-weight:bold'>应到人数</td>";
                        for (var m in data) {
                            for (var n in data[m].list) {
                                tableStr += ' <td>' + data[m].list[n].DueNumber + '</td>';
                            }
                        }
                        tableStr += "  </tr><tr><td style='font-weight:bold'>实到人数</td>";
                        for (var a in data) {
                            for (var b in data[a].list) {
                                tableStr += ' <td>' + data[a].list[b].AttendanceNumber + '</td>';
                            }
                        }
                        tableStr += "  </tr><tr><td style='font-weight:bold'>实到人数比例（%）</td>";
                        for (var c in data) {
                            for (var d in data[c].list) {
                                tableStr += ' <td>' + data[c].list[d].AttendancePro + '</td>';
                            }
                        }
                        tableStr += "  </tr><tr><td style='font-weight:bold'>缺席人数</td>";
                        for (var e in data) {
                            for (var f in data[e].list) {
                                tableStr += ' <td>' + data[e].list[f].MissingNumber + '</td>';
                            }
                        }
                        tableStr += "  </tr><tr><td style='font-weight:bold'>缺席人数比例（%）</td>";
                        for (var x in data) {
                            for (var y in data[x].list) {
                                tableStr += ' <td>' + data[x].list[y].MissingPro + '</td>';
                            }
                        }
                        tableStr += "</tr>";
        ...
        DownLoad: function () {
            ...
            window.location.href = "/Training/AttendanceStaticDown?model=" + JSON.stringify(ajaxDate);
        },
